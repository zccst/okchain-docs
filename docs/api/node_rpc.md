...
Users can operate their own nodes and interact with OKChain via them in their local networks.
...
RPC is built using Tendermint's RPC library. For details, please refer to the documentation and testing document on Tendermint. Link: https://github.com/tendermint/tendermint/tree/master/rpc/lib
...
The JSONRPC request can be sent via websocket. The request can be accessed through gosdk and returned to the corresponding struct.
...
Example: take the information on all trading pairs on a public chain as an example ——`/custom/token/tokenpair`
...
Note: The returned object is amino encoded. If you need to extract the information on its inner members, please amino decode it.
...
Example: The type of height parameters entered in the form of rpcClient.Block should be *int64
...
Example: The type of height parameters entered in the form of rpcClient.BlockResults should be *int64. Return to the latest information on blocks if the height parameters are nil.
...
Example:minHeight,maxHeight are the upper and lower height limits for query. The type should be int64.
...
Example: txHash should be a byte slice decoded via hex entered in the form of rpcClient.Tx. prove is a boolean and is set to be false.
...
Note: Many pieces of data in the returned results are amino encoded. If you need to extract their inner information, you should amino decode them manually.
...
Example: query is a string for height query entered in the form of rpcClient.TxSearch. prove is a boolean and is set to be false. page and perPage are set to be 1 and 30 respectively.
